...
| `monitoring/` | Admin-only diagnostics: `/api/monitoring/presence`, `/api/monitoring/usage` (persisted connection analytics), and the orderbook-fleet statistics trio `/api/monitoring/orderbook` + `/orderbook/history` + `/orderbook/books` (`DescriptiveStats`, `OrderBookStatsPublisher`/`Collector`/`Service`). The trend ring is its own endpoint because at its 720-point cap it dwarfs every statistic in the fleet response. |
...
- **Monitoring the books**: the `TreeMap`s must never be read from another thread — not even copied. Each consumer publishes an immutable `BookStats` record per owned book on a ~1 s cadence (`ShardStatsPublisher`, triggered from `DepthEventHandler` behind a 256-event counter mask, *not* `endOfBatch`); every reader loads that one volatile reference. The hot-path cost of the whole monitoring feature is one `long` increment per applied diff plus one per resync — no allocation, no clock read, no volatile write per message.
